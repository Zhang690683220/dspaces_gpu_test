...
int timer_cb(dspaces_client_t client, struct dspaces_req* req, void* timer) {
    ...
    double put_time_sync = timer_ptr->stop();
    std::cout<< "DSPACES_CPU_PUT_LOCAL() Version = "<< req->ver << " TIME(Sync) = " << put_time_sync << "(ms)" << std::endl;
    return 0;
}
...
int main(int argc, char* argv[]) {
    ...
    gpu_cpu_data = (double*) malloc(size*sizeof(double));

    for(int ts=1; ts<=DEFAULT_TIMESTEP; ts++) {

    #pragma acc parallel loop collapse(3) copy(gpu_cpu_data[0:size])
    for(int i=0; i<dim0; i++) {
        for(int j=0; j<dim1; j++) {
            for(int k=0; k<dim2; k++) {
                gpu_cpu_data[i*dim1*dim2+j*dim2+k] = DEFAULT_VALUE;
            }
        }
    }
    ...
    timer_sync.start();
    timer_async.start();
    dspaces_put_local(ndcl, var_name, ts, sizeof(double), ndims, lb, ub, gpu_cpu_data);
    put_time_async = timer_async.stop();

    std::cout<< "DSPACES_CPU_PUT_LOCAL() Version = "<< ts << " TIME(ASync) = " << put_time_async << "(ms)" << std::endl;
