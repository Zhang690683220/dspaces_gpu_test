...
#include "CLI11.hpp"
#include "mpi.h"
#include "gpu_cpu_put_local.hpp"

void print_usage()
{
    std::cerr<<"Usage: gpu_cpu_put_local --dims <dims> --np <np[0] .. np[dims-1]> --sp <sp[0] ... sp[dims-1]> "
...
int main(int argc, char* argv[]) {
    ...
    CLI::App app{"GPU-CPU PUT LOCAL Emulator for DataSpaces"};
    ...
    int elem_type = 1;
    int num_vars = 1;
    std::string log_name = "gpu_cpu_put_local.log";
    int delay = 0;
    int interval = 1;
